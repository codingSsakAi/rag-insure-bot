...
def index(request):
    # index 템플릿 없으면 작성 폼으로
    try:
        return render(request, "accident_project/index.html")
    except Exception:
        return redirect("accident_project:agreement_new")
...
def agreement_print(request: HttpRequest, pk: int):
    ...
    return render(
        ...
        {
            "F": F,
            "F_json": json.dumps(F, ensure_ascii=False),  # ★ json_script 없이 안전 주입
            "pk": rec.pk,
            "mask_rrn": mask,
            "part_anchors": PART_ANCHORS,
            "label_colors": LABEL_COLORS,
...
def record_print(request, pk: int):
    rec = get_object_or_404(AccidentRecord, pk=pk, owner=request.user)
    F = rec.payload or {}
    ctx = {"F": F, "pk": rec.pk, "F_json": json.dumps(F, ensure_ascii=False)}  # ★
...
def agreement_edit(request, pk):
    ...
    if getattr(ag, "owner_id", None) is None:
        ag.owner = request.user; ag.save(update_fields=["owner"])
    ...
    F["a_parts"] = _arr("a_parts_json"); F["b_parts"] = _arr("b_parts_json")
    F["a_markers"] = _arr("a_markers_json"); F["b_markers"] = _arr("b_markers_json")
    ctx = {
        "F": F,
        "part_anchors": PART_ANCHORS,
        "label_colors": LABEL_COLORS,
...
def agreement_preview_saved(request, pk):
    ...
    if getattr(rec, "owner_id", None) is None:
        rec.owner = request.user; rec.save(update_fields=["owner"])
    if rec.owner_id != request.user.id:
